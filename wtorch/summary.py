...
def add_images_with_label(tb,name,image,label,global_step):
    ...
    if not isinstance(label,Iterable):
        label = str(label)
        image[0] = _draw_text_on_image(image[0], label)
    elif len(label) == 1:
        label = str(label[0])
        image[0] = _draw_text_on_image(image[0],label)
    elif len(label) == image.shape[0]:
        for i in range(len(label)):
            image[i] = _draw_text_on_image(image[i], str(label[i]))
    else:
...
def add_video_with_label(tb,name,video,label,global_step,fps=4):
    ...
    for i in range(video.shape[0]):
        ...
        for j in range(video.shape[1]):
            _draw_text_on_image(video[i,j],l)
