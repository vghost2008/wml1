...
from .en_utils import *

GlobalParams = collections.namedtuple('GlobalParams', [
    'batch_norm_momentum', 'batch_norm_epsilon', 'dropout_rate', 'data_format',
    'num_classes', 'width_coefficient', 'depth_coefficient', 'depth_divisor',
    'min_depth', 'drop_connect_rate', 'relu_fn', 'batch_norm', 'use_se',
    'local_pooling', 'condconv_num_experts', 'clip_projection_output','kernel_regularizer'
])
...
class MBConvBlock(tf.keras.layers.Layer):
  ...
  def __init__(self, block_args, global_params):
    ...
    self.depthwise_conv_cls = DepthwiseConv2D
  ...
  def _build(self):
    ...
    if self._block_args.super_pixel == 1:
      self._superpixel = tf.layers.Conv2D(
          ...
          kernel_regularizer=self._global_params.kernel_regularizer,
          padding='same',
    ...
    self._expand_conv = self.conv_cls(
        ...
        data_format=self._data_format,
        use_bias=False)
    self._bn0 = partial(self._batch_norm,
        decay=self._batch_norm_momentum,
        epsilon=self._batch_norm_epsilon)
    ...
    self._depthwise_conv = self.depthwise_conv_cls(
        ...
        depthwise_regularizer=self._global_params.kernel_regularizer,
        padding='same',
        data_format=self._data_format,
        use_bias=False)

    self._bn1 = partial(self._batch_norm,
                        decay=self._batch_norm_momentum,
                        epsilon=self._batch_norm_epsilon)
    ...
    self._project_conv = self.conv_cls(
        ...
        data_format=self._data_format,
        use_bias=False)
    self._bn2 = partial(self._batch_norm,
                        decay=self._batch_norm_momentum,
                        epsilon=self._batch_norm_epsilon)
  ...
  def call(self, inputs, training=True, drop_connect_rate=None):
    ...
    if self._block_args.fused_conv:
      ...
      x = self._relu_fn(self._bn1(fused_conv_fn(x), is_training=training))
      logging.info('Conv2D: %s shape: %s', x.name, x.shape)
    else:
      # Otherwise, first apply expansion and then apply depthwise conv.
      if self._block_args.expand_ratio != 1:
        x = self._relu_fn(self._bn0(expand_conv_fn(x), is_training=training))
        logging.info('Expand: %s shape: %s', x.name, x.shape)

      x = self._relu_fn(self._bn1(depthwise_conv_fn(x), is_training=training))
    ...
    x = self._bn2(project_conv_fn(x), is_training=training)
  ...
  def _build(self):
    ...
    if self._block_args.expand_ratio != 1:
      ...
      self._expand_conv = tf.layers.Conv2D(
          ...
          strides=[1, 1],
          kernel_initializer=conv_kernel_initializer,
          kernel_regularizer=slim.l2_regularizer(1e-4),
          padding='same',
          use_bias=False)
...
class Model(tf.keras.Model):
  ...
  def _build(self):
    ...
    self._conv_stem = tf.layers.Conv2D(
        ...
        use_bias=False)
    self._bn0 = partial(self._batch_norm, decay=batch_norm_momentum,
  ...
  def call(self,
    ...
    with tf.variable_scope('stem'):
      outputs = self._relu_fn(
          self._bn0(self._conv_stem(inputs), is_training=training))
    ...
    if not features_only:
      ...
      with tf.variable_scope('head'):
        outputs = self._relu_fn(
            self._bn1(self._conv_head(outputs), is_training=training))
