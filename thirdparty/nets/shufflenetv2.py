...
def shufflenetv2(images, is_training, num_classes=1000, depth_multiplier='1.0',use_max_pool=False):
    ...
    def batch_norm(x):
        ...
        return x
    end_points = {}
    with tf.variable_scope('ShuffleNetV2') as sc:
        params = {
        ...
        with slim.arg_scope([slim.conv2d, depthwise_conv], **params):
            ...
            x = slim.conv2d(images, 24, (3, 3), stride=2, scope='Conv1')
            end_points[sc.name+"/Stage1"] = x
            if use_max_pool:
                #原实现默认会进行max_pool, 为了保持高的分辨率，这里默认改成了不max pool
