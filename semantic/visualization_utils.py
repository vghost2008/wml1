...
mask:[height,width], value 0/1
'''
def tf_draw_mask_on_image_array(image, mask, color='red', alpha=0.4):
...
alpha:
'''
def tf_draw_masks_on_images(image, mask, color=u"red",alpha=0.4,name='summary_image_with_mask'):
    if image.dtype is not tf.uint8:
        image = tf.cast(image,tf.uint8)
    if mask.dtype is not tf.uint8:
        mask = tf.cast(mask,tf.uint8)
    with tf.name_scope(name):
        def fn(img,msk):
            return tf_draw_mask_on_image_array(img,msk,color,alpha)

        image = tf.map_fn(lambda x:fn(x[0],x[1]),elems=(image,mask),dtype=(tf.uint8),back_prop=False)
