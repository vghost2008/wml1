...
TARGET_IMG_WIDTH = 480
TARGET_IMG_HEIGHT = 256
TARGET_INSTANCE_SIZE = 72
TARGET_IMG_SIZE = TARGET_IMG_HEIGHT*TARGET_IMG_HEIGHT
    ...
    def get_data_items(self):
        for seq_root in self.dirs:
            ...
            for seq in seqs:
                ...
                #get mean instance bbox size
                all_sizes = []
                for v in gt:
                    fid, tid, x, y, w, h, mark, label, _ = v[:9]
                    all_sizes.append(math.sqrt(w*h))
                mean_size = np.mean(all_sizes)
                new_size = self.get_img_size(seq_width,seq_height,mean_size)
                fid_datas = {}  #frame id->datas
                for v in gt:
                    fid, tid, x, y, w, h, mark, label, _ = v[:9]
                    if mark == 0:
                        continue
                ...
                for fid,datas in fid_datas.items():
                    ...
                    if len(tmp_data)>0:
                        ...
                        yield img_data,tmp_data,new_size

        print(f"Last tid value {self.tid_curr}")

    def get_img_size(self,img_w,img_h,mean_size):
        scal_ratio = min(TARGET_INSTANCE_SIZE/mean_size,1.0)

        new_w = int(img_w*scal_ratio)
        new_h = int(img_h*scal_ratio)

        if new_h<TARGET_IMG_HEIGHT or new_w<TARGET_IMG_WIDTH:
            scal_ratio = min(max(TARGET_IMG_HEIGHT/img_h,TARGET_IMG_WIDTH/img_w),1.0)
            new_w = int(img_w*scal_ratio)
            new_h = int(img_h*scal_ratio)
        return new_w,new_h
...
def create_tf_example(image,
    ...
    img = wmli.resize_img(img,new_size,keep_aspect_ratio=True)
...
def _create_tf_record(
    ...
    for idx,data in enumerate(datasets.get_data_items()):
        img_data,bboxes_datas,new_size = data
        ...
        tf_example = create_tf_example(
            img_data,bboxes_datas,new_size)
