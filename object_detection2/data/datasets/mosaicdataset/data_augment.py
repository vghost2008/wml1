...
def preproc(img, input_size, swap=(2, 0, 1),hsv_prob=None):
    '''
    keep aspect ratio resize img to input_size (pad 114 if need), trans img channels
    '''
    if len(img.shape) == 3:
    ...
    if hsv_prob is not None and random.random() < hsv_prob:
        augment_hsv(padded_img)
...
def preproc_no_pad(img, input_size, swap=(2, 0, 1)):
    ...
    resized_img = cv2.resize(
        img,
        (int(img.shape[1] * r)//32*32, int(img.shape[0] * r)//32*32),
        interpolation=cv2.INTER_LINEAR,
    ).astype(np.uint8)
...
class TrainTransform:
    ...
    def __call__(self, image, targets, input_dim):
        '''
        image: 
        targets: [N,5] [xmin,ymin,xmax,ymax,label]
