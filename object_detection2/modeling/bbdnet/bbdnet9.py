...
from .abstractbbdnet8 import AbstractBBDNet
...
class BBDNetForOneImg(AbstractBBDNet):
    ...
    def build_net(self):
        ...
        with tf.variable_scope("NodeEncode"):
            ...
            # Process map data
            net2 = slim.fully_connected(self.map_data, self.POINT_HIDDEN_SIZE // 2)
...
class BBDNet9(WModule):
    ...
    def process_one_image(self,datas):
        ...
        if self.is_training:
            ...
            nms_probs,nms_mask = bbd_net.get_nms_predict(threshold=0.3)
            p_labels = tf.cast(nms_mask,tf.int32)*10
            nms_y, nms_y_scores,nms_indexs = wop.boxes_match_with_pred(boxes=tf.expand_dims(raw_bboxes,axis=0),
                                                    plabels=tf.expand_dims(p_labels,axis=0),
                                                    pprobs=tf.expand_dims(nms_probs,axis=0),
                                                    gboxes=tf.expand_dims(datas[GT_BOXES],axis=0),
                                                    glabels=tf.expand_dims(datas[GT_LABELS],axis=0),
                                                    glens=tf.reshape(datas[GT_LENGTH],[1]),
                                                    is_binary_plabels=True,
                                                    threshold=0.5)
