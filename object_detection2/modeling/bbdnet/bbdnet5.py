...
from .abstractbbdnet5 import AbstractBBDNet
...
class BBDNetForOneImg(AbstractBBDNet):
    ...
    def build_net(self):
        ...
        edge_fn_i = partial(slim.fully_connected, num_outputs=self.EDGE_HIDDEN_SIZE, normalizer_fn=self.normalizer_fn,scope="UpdateEdge")
        point_fn_i = partial(slim.fully_connected, num_outputs=self.POINT_HIDDEN_SIZE, normalizer_fn=self.normalizer_fn,scope="UpdatePoint")
        if self.cfg.MODEL.BBDNET.USE_GLOBAL_ATTR:
            ...
            global_fn_i = partial(slim.fully_connected,num_outputs=self.POINT_HIDDEN_SIZE, normalizer_fn=self.normalizer_fn,scope="UpdateGlobal")
        else:
            global_fn = None
            global_fn_i = None
