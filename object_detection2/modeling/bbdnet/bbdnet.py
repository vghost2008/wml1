...
from .abstractbbdnet import AbstractBBDNet
...
class BBDNetForOneImg(AbstractBBDNet):
    ...
    def __init__(self, cfg,boxes, probability, map_data, classes_num, base_net, is_training=False, rnn_nr=1):
    ...
    def build_net(self):
        ...
        if len(self.map_data.get_shape()) == 4:
            with tf.variable_scope("smooth_map_data"):
                ...
                B,H,W,C = wmlt.combined_static_and_dynamic_shape(map_data)
                map_data = tf.reshape(map_data,[B,H*W*C])
        ...
        with tf.variable_scope("NodeEncode"):
            ...
            net0 = slim.fully_connected(raw_points, POINT_HIDDEN_SIZE // 2)

            # Process map data
            net2 = BBDNetForOneImg.self_attenation(tf.expand_dims(self.map_data, axis=0), n_head=2,
                                              is_training=self.is_training, normalizer_fn=wnnl.layer_norm)
            net2 = tf.squeeze(net2, axis=0)
            net2 = slim.fully_connected(net2, POINT_HIDDEN_SIZE // 2)
            # Fusion all parts of node.
            net = tf.concat([net0, net2], axis=1)
            #net = tf.zeros_like(net)
            points_data = AbstractBBDNet._mlp(net, dims=POINT_HIDDEN_SIZE, scope="MLP_a")

        edge_fn = partial(AbstractBBDNet.res_block, dims=EDGE_HIDDEN_SIZE, unit_nr=4, scope="UpdateEdge")
        point_fn = partial(AbstractBBDNet.res_block, dims=POINT_HIDDEN_SIZE, unit_nr=4, scope="UpdatePoint")
        edge_fn_i = partial(AbstractBBDNet._mlp, dims=EDGE_HIDDEN_SIZE, scope="UpdateEdge", pool=False)
        point_fn_i = partial(AbstractBBDNet._mlp, dims=POINT_HIDDEN_SIZE, scope="UpdatePoint", pool=False)
        if self.cfg.MODEL.BBDNET.USE_GLOBAL_ATTR:
            global_fn = partial(AbstractBBDNet.res_block, dims=POINT_HIDDEN_SIZE, unit_nr=4, scope="UpdateGlobal")
            global_fn_i = partial(AbstractBBDNet._mlp, dims=POINT_HIDDEN_SIZE, scope="UpdateGlobal", pool=False)
        else:
            global_fn = None
            global_fn_i = None
        ...
        if self.cfg.MODEL.BBDNET.USE_GLOBAL_ATTR:
            with tf.variable_scope("InitGlobalAttr"):
                ...
                self.A.update_global_independent(partial(AbstractBBDNet.mlp, dims=GLOBAL_HIDDEN_SIZE, layer=2,scope="GlobalEncode"))
        else:
        ...
        with tf.variable_scope("EdgeEncode"):
            ...
            iou = tf.expand_dims(iou, axis=-1) * tf.ones(shape=[1, EDGE_HIDDEN_SIZE // 2])
            ...
            e_data0 = AbstractBBDNet._mlp(e_data0, scope="MLP_a", dims=EDGE_HIDDEN_SIZE // 2)
            ...
            e_data1 = AbstractBBDNet._mlp(e_data1, scope="MLP_b", dims=EDGE_HIDDEN_SIZE // 2)
            e_data = tf.concat([e_data0, e_data1, iou], axis=1)
            e_data = AbstractBBDNet.mlp(e_data, dims=EDGE_HIDDEN_SIZE, layer=2, scope="EdgeEncode")
        ...
        def output_fn(net):
            with tf.variable_scope("NodesOutput", reuse=tf.AUTO_REUSE):
                ...
                net = slim.fully_connected(net,  POINT_HIDDEN_SIZE // 2)
        ...
        def edge_output_fn(net):
            with tf.variable_scope("EdgesOutput", reuse=tf.AUTO_REUSE):
                # net = AbstractBBDNet.max_pool(net)
                net = slim.fully_connected(net,  EDGE_HIDDEN_SIZE // 2)
        ...
        if self.cfg.MODEL.BBDNET.USE_GLOBAL_ATTR and self.cfg.MODEL.BBDNET.USE_GLOBAL_LOSS:
            def global_output_fn(net):
                with tf.variable_scope("GlobalOutput", reuse=tf.AUTO_REUSE):
                    # net = AbstractBBDNet.max_pool(net)
                    net = slim.fully_connected(net, max(GLOBAL_HIDDEN_SIZE // 2,self.classes_num+1))
                    net = slim.fully_connected(net, self.classes_num+1,
                                               normalizer_fn=None,
                                               activation_fn=None)
                    net = tf.expand_dims(net, axis=0)
        ...
        for i in range(2):
            with tf.variable_scope(f"Layer_{i}"):
                ...
                self.A.update_independent(point_fn_i, edge_fn_i, global_fn_i,
                                          ["UpdatePoint_i", "UpdateEdge_i", "UpdateGlobal_i"])
    ...
    def loss(self, y):
        ...
        if self.cfg.MODEL.BBDNET.USE_EDGE_LOSS:
            with tf.name_scope("edges_loss"):
                ...
                senders_y = tf.cast(tf.greater(senders_y, 0), tf.int32)
                receivers_y = tf.cast(tf.greater(receivers_y, 0), tf.int32)
                edge_y = senders_y * 2 + receivers_y
                scale = 0.05
                for i, logits in enumerate(self.mid_edges_outputs):
                    p_loss, n_loss = AbstractBBDNet._loss(logits, edge_y)
                    loss_list.append(p_loss * scale)
                    loss_list.append(n_loss * scale)

        if self.cfg.MODEL.BBDNET.USE_GLOBAL_LOSS:
            with tf.name_scope("global_loss"):
                gy = wmlt.indices_to_dense_vector(indices=y, size=self.classes_num+1, indices_value=1, dtype=tf.float32)
...
class BBDNet(WModule):
    ...
    def process_one_image(self,datas):
        ...
        outputs = {}
        if self.is_training:
            fboxes, flabels, probs, raw_plabels = bbd_net.get_predict(bboxes)
