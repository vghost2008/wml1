...
from GN.graph import DynamicAdjacentMatrix,DynamicAdjacentMatrixShallow,DynamicAdjacentMatrixAtt
...
from .abstractbbdnetx2 import AbstractBBDNet
...
class BBDNetForOneImg(AbstractBBDNet):
    ...
    def build_net(self):
        ...
        if len(self.map_data.get_shape()) == 4:
            with tf.variable_scope("EncodeMapData"):
                B,R,R,C = wmlt.combined_static_and_dynamic_shape(self.map_data)
                map_data = tf.reshape(self.map_data,[B,R*R*C])
                map_data = slim.fully_connected(map_data,1024,normalizer_fn=wnnl.evo_norm_s0,activation_fn=None)
                map_data = slim.fully_connected(map_data,1024, normalizer_fn=wnnl.evo_norm_s0,activation_fn=None)
        else:
        ...
        self.A_att = DynamicAdjacentMatrixAtt(adj_mt=adj_mt,
                                              points_data=points_data,
                                              edges_data=None,
                                              edges_data_dim=self.EDGE_HIDDEN_SIZE)
        ...
        self.A_att.use_sent_edges_for_node = self.cfg.MODEL.BBDNET.USE_SENT_EDGES_FOR_NODE
        ...
        self.A_att.set_datas(self.A.datas())
        for i in range(self.preprocess_nr):
            with tf.variable_scope(f"Layer_{i}"):
                if i > 0:
                    self.A_att.concat(latent0)
                self.A_att.update_independent(point_fn_i,edge_fn_i,global_fn_i,["UpdatePoint_i","UpdateEdge_i","UpdateGlobal_i"])
                self.A_att.update(point_fn, edge_fn, global_fn, ["UpdatePoint", "UpdateEdge", "UpdateGlobal"])

        latent0 = self.A.datas()
        self.A.set_datas(self.A_att.datas())
